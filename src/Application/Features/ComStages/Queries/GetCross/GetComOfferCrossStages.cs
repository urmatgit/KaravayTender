...
namespace CleanArchitecture.Razor.Application.Features.ComStages.Queries.GetCross
{
...

    {
        ...
        private StagesCrossDto GetProStageInfo(IEnumerable<StagesCrossDto> stage, int compostionId, int ContragentId, int number)
        {

            var result = stage.
                       Where(x => x.ContragentId == ContragentId && x.ComPositionId == compostionId && x.Number == number)
                       .SingleOrDefault();
            return result;
        ...
        private async Task<ComStageCrossDTO> GetCrossDataAllStagePage(int comOfferId, CancellationToken cancellationToken)
        {
            ...
            var AlltageData = await GetDataAllStage(comOfferId, cancellationToken);
            var lastStageNumber = AlltageData.OrderBy(o => o.Number).LastOrDefault();
            var contrgents = (from s in AlltageData
                              group s by new { s.ContragentName, s.ContragentId } into contr
                              orderby contr.Key.ContragentName
                              select new
                              {
                                  caption = $"{contr.Key.ContragentName}",
                                  contrId = contr.Key.ContragentId,
            ...
            var listPosition = AlltageData.GroupBy(g => new { g.ComPositionId, g.ComStageId });
            foreach (var position in listPosition)
                foreach (var nomenclture in AlltageData.Where(p => p.ComPositionId == position.Key.ComPositionId && p.ComStageId == position.Key.ComStageId).Select(n => new
                {
                    n.NomenclatureName,
                    n.DeadlineDate,
                    n.ComPositionId,
                ...
                }).Distinct())
                {
                    ...
                    GoodPriceDto GoodPrice = new GoodPriceDto();
                    //перечисляем контрагентов
                    foreach (var contr in AlltageData.Where(c => c.ComPositionId == nomenclture.ComPositionId && c.ComStageId == nomenclture.ComStageId).OrderBy(o => o.ContragentName))
                    {
                        ...
                        //Indexcontrgent++;
                        Indexcontrgent = contrgents.FindIndex(x => x.contrId == contr.ContragentId) + 1;

                        row.Add($"ContrId{Indexcontrgent}", contr.ContragentId);
                        row.Add($"ComPositionId{Indexcontrgent}", contr.ComPositionId);

                        if (GoodPrice.Price == null || contr.Price <= GoodPrice.Price)
                        {
                            GoodPrice.ContragentId = contr.ContragentId;
                            GoodPrice.Price = contr.Price;
                            GoodPrice.ColorRGB = "rgb(0,176,80)";
                        }
                        row.Add($"ContrPrice{Indexcontrgent}", contr.Price);
                        if (nomenclture.ComStageId == lastStageNumber.ComStageId && nomenclture.Number > 1
                            && (contr.Status != ParticipantStatus.FailureParitipate && contr.Status != ParticipantStatus.Cancel))
                        {
                            var lastprice = GetProStageInfo(AlltageData, contr.ComPositionId, contr.ContragentId, nomenclture.Number - 1);
                            if (lastprice != null && lastprice.Price <= contr.Price)
                            {

                                row.Add($"ContrPriceColor{Indexcontrgent}", "rgb(255,0,0)");
                                if (GoodPrice.ContragentId == contr.ContragentId)
                                {
                                    GoodPrice.ColorRGB = "rgb(255,0,0)";
                                }
                            }

                        }
                        else
                            row.Add($"ContrPriceColor{Indexcontrgent}", "");

                        //if (nomenclture.ComStageId == lastStageNumber.ComStageId)
                        
                            var ContrLastStatus = contrgents.FirstOrDefault(x => x.contrId == contr.ContragentId && x.number== nomenclture.Number);
                            if (ContrLastStatus !=null && (ContrLastStatus.status
                                == ParticipantStatus.FailureParitipate || ContrLastStatus.status == ParticipantStatus.Cancel))
                            {
                                row.Add($"ShowX{Indexcontrgent}", true);
                            }
                        
                        else
                            row.Add($"ShowX{Indexcontrgent}", false);
        ...
        private async Task<ComStageCrossDTO> GetCrossDataLastStagePage(int comOfferId, CancellationToken cancellationToken)
        {
            ...
            var AllstageData = await GetDataAllStage(comOfferId, cancellationToken);
            ...
            foreach (var position in listPosition)
                ...
                }).Distinct())
                {
                    ...
                    GoodPriceDto GoodPrice = new GoodPriceDto();
                    //перечисляем контрагентов
                    foreach (var contr in lastStageData.Where(c => c.ComPositionId == nomenclture.ComPositionId).OrderBy(o => o.ContragentName))
                    {
                        // var status = GetStatus(pos.stage1.stage.StageParticipants, stagecom.ContragentId);
                        // if (status == ParticipantStatus.FailureParitipate && stage==1) continue;
                        Indexcontrgent++;
                        ...
                        if (lastStageNumber.Number > 1
                            && (contr.Status != ParticipantStatus.FailureParitipate && contr.Status != ParticipantStatus.Cancel))
                        {
                            var lastprice = GetProStageInfo(AllstageData, contr.ComPositionId, contr.ContragentId, lastStageNumber.Number - 1);
                            if (lastprice != null && lastprice.Price <= contr.Price)
                            {
                            ...
                            row.Add($"ContrPriceColor{Indexcontrgent}", "");
                        if (contr.Status == ParticipantStatus.FailureParitipate || contr.Status == ParticipantStatus.Cancel)
                        {
                            row.Add($"ShowX{Indexcontrgent}", true);
                        }
                        else
